# -*- coding: utf-8 -*-
"""Checks one page: HTML, dashes, links, assets, contrast, overflow and
text sizes, on desktop and on real mobile via an iframe."""
...
P().feed(raw)
print("  unclosed HTML:", st or "none", " mismatched:", bad or "none")
miss = [a for a in sorted(set(re.findall(r'(?:src|srcset)="(build-assets/[^"]+)"', raw)))
        if not os.path.exists(a)]
print("  missing assets:", miss or "none")
...
print("  broken links:", brk or "none")
...
<script>setTimeout(function(){
 ...
 o.push('height '+R.scrollHeight+'  overflows '+(R.scrollWidth>R.clientWidth)+' ('+R.scrollWidth+'/'+R.clientWidth+')');
 o.push('sections '+D.querySelectorAll('section').length+'  h1 '+D.querySelectorAll('h1').length+'  h2 '+D.querySelectorAll('h2').length+'  h3 '+D.querySelectorAll('h3').length+'  details '+D.querySelectorAll('details').length);
 var bi=[];D.querySelectorAll('img').forEach(function(i){if(!i.complete||i.naturalWidth===0)bi.push(i.currentSrc||i.src)});
 o.push('images '+D.querySelectorAll('img').length+'  failed '+(bi.length?bi.join(','):'0'));
 ...
 o.push('text <12px: '+(sm.length?sm.slice(0,4).join(', '):'none'));
 ...
 o.push('elements past the width: '+(ov.length?ov.slice(0,4).join(' | '):'none'));
 ...
 D.querySelectorAll('body *').forEach(function(e){
   ...
   if(r<worst){worst=r;wd=r.toFixed(2)+' ('+(lg?'large':'normal')+' '+Math.round(fs)+'px) '+pth(e)}
   if(r<need)fail.push(r.toFixed(2)+'/'+need+' '+Math.round(fs)+'px '+pth(e)+' "'+e.textContent.trim().slice(0,28)+'"');
 });
 o.push('contrast: '+Object.keys(seen).length+' pairs, AA failures '+fail.length+'  |  lowest in use '+wd);
 fail.forEach(function(x){o.push('   FAIL '+x)});
